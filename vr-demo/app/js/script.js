...
// checking if there is a parameter in the URL / add one if not
...
// navigate to new location and fade out old location out / assume entrance if no old location
function navigate(loc) {

    var skyObj = document.querySelector('a-sky').components.material;
    var oldNav = null;
    
    if ( typeof skyObj === 'undefined' ) {
        oldNav = "nav_entrance"
    } else {
...
AFRAME.registerComponent('marker', {
    schema: {
        icon: {type: 'string', default: 'icn_point'},
        enterColor: {type: 'string', default: '#ffffff'},
        leaveColor: {type: 'string', default: '#ffffff'},
        yRot: {type: 'number', default: 0},
        yPos: {type: 'number', default: 0},
        zPos: {type: 'number', default: -3}
    },
    init: function () {
        var data = this.data;
        var el = this.el;
...
AFRAME.registerComponent('enter-room', {
    schema: {
        ...
        target: {type: 'selector', default: 'a-sky'},
        src: {type: 'string'},
